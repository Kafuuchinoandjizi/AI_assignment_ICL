...
class ComparativeAnalysis:
    ...
    def _generate_analysis(self, df: pd.DataFrame) -> Dict[str, str]:
        ...
        # 时间效率分析
        time_accuracy_corr = df["准确率"].corr(df["平均响应时间"])
        analysis["time_analysis"] = f"准确率与响应时间相关性: {time_accuracy_corr:.2f}"
        
        # 策略效果分析
        best_strategy = df.loc[df["综合评分"].fillna(0).idxmax()] if not df["综合评分"].isna().all() else df.iloc[0]
...
def test_evaluation():
    ...
    # 测试数据
    test_questions = [
        {"question": "2 + 3等于多少？", "answer": "5"},
        {"question": "10除以2等于多少？", "answer": "5"},
        {"question": "一个篮子里有6个苹果，拿走2个，还剩几个？", "answer": "4"}
    ]
    
    # 评估不同策略
    for strategy_name in ["zero_shot", "few_shot", "zero_shot_cot"]:
        print(f"\n评估策略: {PROMPT_STRATEGIES[strategy_name].name}")
        result = evaluator.evaluate_strategy(strategy_name, "arithmetic_reasoning", test_questions)
