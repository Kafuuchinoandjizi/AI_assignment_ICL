...
# 保持此设置，用于控制Hugging Face Hub的下载超时和重试
...
# ====================== 数据集加载（移除 load_dataset 中的 timeout 参数） ======================
...
def normalize_math_answer(ans: str) -> str:
    """
    增强答案归一化：处理 CoT 提取、单位、分数、浮点数。
    统一转换为浮点数字符串，以确保 109 == 109.0 成立。
    """
    if not ans:
        return ""
    # 1. 尝试从 CoT 格式中提取答案（####）
    ...
    # 4. 尝试转换为浮点数进行统一处理
    try:
        # 统一使用 float() 转换，保证 109 和 109.0 统一比较
...
def evaluate_ner(preds: List[str], refs: List[str]) -> Dict[str, float]:
    ...
    # ------------------ 修复点：直接获取 input 列的前2个元素 ------------------
    # 避免在迭代 Dataset slice 时可能出现的类型问题
    input_texts_for_validation = test_data["input"][:2]

    for i, (pred_str, ref_str, input_text) in enumerate(
            zip(preds[:2], refs[:2], input_texts_for_validation)):
        # ----------------------------------------------------------------------
        ...
        for ent_text, ent_type in pred_entities:
            ...
            for j in range(len(tokens) - len(ent_tokens) + 1):
                if tokens[j:j + len(ent_tokens)] == ent_tokens:
                    pred_tags[j] = f"B-{ent_type}"
                    for k in range(1, len(ent_tokens)):
                        pred_tags[j + k] = f"I-{ent_type}"
                    break  # 找到第一个匹配就退出，避免重复标记
        for ent_text, ent_type in ref_entities:
            ent_tokens = ent_text.split()
            for j in range(len(tokens) - len(ent_tokens) + 1):
                if tokens[j:j + len(ent_tokens)] == ent_tokens:
                    ref_tags[j] = f"B-{ent_type}"
                    for k in range(1, len(ent_tokens)):
                        ref_tags[j + k] = f"I-{ent_type}"
                    break

        all_pred_tags.append(pred_tags)
        all_ref_tags.append(ref_tags)

    # 修复：确保即使只有部分样本也被计入有效计数
    f1 = f1_score(all_ref_tags, all_pred_tags, average="micro") * 100 if len(all_ref_tags) > 0 else 0.0
...
def run_experiment(task_name: str, strategy: str) -> Dict[str, any]:
    ...
    # 将所有样本加载到 test_data，以便 evaluate_ner 能访问 input_text
    ...
    for i, example in enumerate(test_data):
        ...
        try:
            ...
            if task_name == "sentiment_classification":
                ...
                # 对于 zero-shot/few-shot，pred.answer 可能未经 normalize_math_answer 处理，这里确保记录原始输出
            ...
            # 统计Token：从模块的 current_prompt 获取
...
def print_results_table(results: List[Dict[str, any]]):
    ...
    # 使用实际运行的样本数
    sample_size = valid_results[0]['sample_size'] if valid_results else TEST_SAMPLE_SIZE

    print("\n" + "=" * 120)
    print(f"📊 实验结果汇总（样本数：{sample_size}，模型：{OLLAMA_MODEL_NAME}）")
    print("=" * 120)
...
def interactive_demo():
    ...
    if task_name not in datasets_dict:
        print(f"❌ 数据集 {task_name} 加载失败，无法进行 Demo。")
        return
    ...
    try:
        ...
        if task_name == "sentiment_classification":
            ...
            # 统一使用归一化后的答案进行展示
            norm_answer = normalize_math_answer(str(pred.answer))
            print(f"答案：{norm_answer} (原始输出: {pred.answer})")
...
if __name__ == "__main__":
    # 使用 global 声明 test_data，以便在 evaluate_ner 中访问
    test_data = None
